...
int main(){
	...
	big B(1); A[0][1] = 1;
	for (int i=1; i<367; i++) {
		B=B*i; for(int j=0;j<10;j++) A[i][j]=0;
		ostringstream os; os<<B;
		string s=os.str();
	...
	while (cin>>N && N ) {
		cout<<N<<"! --"<<endl;
		for(int i=0;i<5;i++)cout<<"   ("<<i<<")"<<setw(5)<<setfill(' ')<<A[N][i]; cout<<endl;
		for(int i=5;i<10;i++)cout<<"   ("<<i<<")"<<setw(5)<<setfill(' ')<<A[N][i]; cout<<endl;
	}
}